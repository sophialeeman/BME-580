...
clusters <- hclust(dist(subset(df_f[numeric_cols_f], select = -c(Case_Fatality_Ratio, Incident_Rate, Confirmed, Deaths, Total_Test_Results, Testing_Rate, Year))))
plot(clusters)
...
rect.hclust(clusters, k = 2, border = outcome_colors)
...
clusters2 <- hclust(dist(subset(df_i[numeric_cols_i], select = -c(Case_Fatality_Ratio, Incident_Rate, Confirmed, Deaths, Total_Test_Results, Testing_Rate, Year))))
plot(clusters2)
# Define colors based on outcome variable
outcome_var <- df_i$Incident_Rate
outcome_colors <- ifelse(outcome_var > mean(outcome_var), "red", "blue") 
rect.hclust(clusters2, k = 3, border = outcome_colors)
...
library(plotly)
# Plot the data with plotly
fig <- plot_ly(df_i, x = ~PC1, y = ~PC2, z = ~PC3,
